...
def main():
  ...
  # Images to init scene i.e. to estimate 
  # rel. pose between the first images from essential matrix E
  init_images = [1,2]
  ...
  # assume Identity for camera pose of im1
  e_im2.SetPose(np.eye(3),[0,0,0])
  points3D_in_front = []
  for Ri,ti in possible_relative_poses:
    # set current guess of pose in im2
    e_im1.SetPose(Ri,ti)
    points3D, _, _ = TriangulatePoints(K, e_im1, e_im2, e_matches)
    # add number of 3D points in front of camera to list
    points3D_in_front.append(points3D.shape[0])
  
  # find pose with max points in front of both cameras
  idx_max = np.argmax(points3D_in_front)
  R = possible_relative_poses[idx_max][0]
  t = possible_relative_poses[idx_max][1]
  ...
  ax.scatter(points3D[:,0],points3D[:,1],points3D[:,2],c='b',s=1)
  plt.title("Reconstruction after first match with essential matrix")
  plt.show()
