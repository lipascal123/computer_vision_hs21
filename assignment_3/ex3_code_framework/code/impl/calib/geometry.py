...
def DecomposeP(P):
  ...
  # Find K and R
  # P: 3x4
  # print("P[0:3, 0:3]",P[0:3, 0:3])
  R_inv, K_inv = np.linalg.qr(np.linalg.inv(P[0:3, 0:3]))
  ...
  K = np.dot(K,T)
  R = np.dot(np.linalg.inv(T),R)

  # TODO
  # Find the camera center C as the nullspace of P
  C_nN = null_space(P)
  # Nullspace should be unique
  assert( C_nN.shape[1] == 1)
  C = HNormalize(C_nN)
